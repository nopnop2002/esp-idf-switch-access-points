...
esp_err_t wifi_connect_sta(char * ssid, char * password)
{
	...
	wifi_config_t wifi_config = {
		.sta = {
			.ssid =_ESP_WIFI_SSID1,
			.password = ESP_WIFI_PASSWORD1,
...
void app_main(void)
{
	...
	for(int i=0;i<5;i++) {
		...
		if (wifi_connect_sta(CONFIG_ESP_WIFI_SSID1, CONFIG_ESP_WIFI_PASSWORD1) == ESP_OK) {
			...
			ESP_LOGE(TAG, "Unable to connect to %s", CONFIG_ESP_WIFI_SSID1);
		...
		if (wifi_connect_sta(CONFIG_ESP_WIFI_SSID2, CONFIG_ESP_WIFI_PASSWORD2) == ESP_OK) {
			...
			ESP_LOGE(TAG, "Unable to connect to %s", CONFIG_ESP_WIFI_SSID2);
			while(1) { vTaskDelay(1); }
		}
	}
